...
export const RestoreModalContext = createContext<RestoreModalContextProps | undefined>(undefined);
export const RestoreModalProvider: FC<{ children: ReactNode }> = ({ children }) => {
  const [isRestoreModalOpen, setIsRestoreModalOpen] = useState(false);
  const [restoreTransactionFunction, setRestoreTransactionFunction] = useState<() => void>(
    () => {}
  );
...
export const useRestoreModal = () => {
  ...
  if (!context) {
    throw new Error('useRestoreModal must be used within a RestoreModalProvider');
  }
  return context;
};
