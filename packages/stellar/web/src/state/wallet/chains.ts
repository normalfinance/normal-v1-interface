...
export const allChains: WalletChain[] = [public_chain, futurenet, testnet, sandbox, standalone];
...
export const networkToActiveChain = (networkDetails: any, chains: any) => {
  ...
  const supported = chains.find(
    (chain: any) => chain.networkPassphrase === networkDetails?.networkPassphrase
  );
