...
async function liquidityHandler(req: NextRequest) {
  try {
    const { walletAddress } = await req.json();
    if (!walletAddress) {
      await logWithConfig('warn', 'Liquidity API called without wallet address');
      return NextResponse.json({ error: 'Missing walletAddress' }, { status: 400 });
    }

    // Get Edge Config for rate limiting
    const rateLimitConfig = await getRateLimitConfig('liquidity');
    const apiConfig = await getApiConfig('liquidity');
    ...
    if (!success) {
      await logWithConfig('warn', 'Rate limit exceeded for liquidity API', { walletAddress });
...
export const POST = createEdgeConfigHandler(liquidityHandler, 'liquidity');
