...
async function tokensHandler(req: NextRequest) {
  try {
    const { walletAddress } = await req.json();
    if (!walletAddress) {
      await logWithConfig('warn', 'Tokens API called without wallet address');
      return NextResponse.json({ error: 'Missing walletAddress' }, { status: 400 });
    }

    // Get Edge Config for rate limiting
    const rateLimitConfig = await getRateLimitConfig('tokens');
    const apiConfig = await getApiConfig('tokens');
    ...
    if (!success) {
      await logWithConfig('warn', 'Rate limit exceeded for tokens API', { walletAddress });
...
export const POST = createEdgeConfigHandler(tokensHandler, 'tokens');
