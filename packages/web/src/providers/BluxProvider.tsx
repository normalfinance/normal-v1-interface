...
export function BluxProvider({ children }: BluxProviderProps) {
  ...
  try {
    ...
    const provider = (
      <BluxSDKProvider config={configToUse}>
        {children}
      </BluxSDKProvider>
    );
    ...
    console.error('❌ BluxProvider: Error stack:', error instanceof Error ? error.stack : 'No stack');
    console.error('❌ BluxProvider: Error name:', error instanceof Error ? error.name : 'Unknown');
    console.error('❌ BluxProvider: Error message:', error instanceof Error ? error.message : 'Unknown');
