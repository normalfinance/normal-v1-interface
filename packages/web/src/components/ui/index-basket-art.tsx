...
import { m, domMax, useInView, LazyMotion, useReducedMotion } from 'framer-motion';

/* ----------------------------- Types & Props ----------------------------- */

type CoinId = 'BTC' | 'DOGE' | 'XRP' | 'ADA' | 'ETH';
...
// visual depths and sideways arcs (tuned for 271x303 viewBox)
const DEFAULT_DOWN: Record<CoinId, number> = {
  BTC: 84,
  DOGE: 110,
  XRP: 118,
  ADA: 169,
...
const DEFAULT_SIDE: Record<CoinId, number> = {
  BTC: -8,
  DOGE: 20,
  XRP: -38,
  ADA: -32,
...
// default timing
const DURATION = 4.6;
const UP_START_T = 0.72;
const STAGGER_STEP = 0.05;
const FALL_FRAC = 0.22;

/* ----------------------------- Helper: arcs ------------------------------ */
/**
 * Build per-coin keyframes:
 * - waits until t0
 * - falls until t1
 * - waits at bottom until upStart (simultaneous rise)
 * - rises to top by t=1
 */
function buildKeyframes(
  ...
  const t1 = Math.min(t0 + fallFrac, upStart - 0.04); // ensure tiny dwell before upStart
  const x = [0, sideX, sideX, 0];
  const y = [0, downY, downY, 0];
  const times = [Math.max(0, t0), Math.max(0, t1), upStart, 1];
  return { x, y, times };
}
...
export default function IndexBasketArt({
  ...
  ariaLabel = 'Coins flowing into a basket index',
}: Props) {
  const uid = React.useId();
  const clipId = React.useMemo(() => `cp1-${uid.replace(/[:]/g, '')}`, [uid]);

  const prefersReduced = useReducedMotion();

  // Pause animation when offscreen (optional)
  const ref = React.useRef<SVGSVGElement | null>(null);
  const inView = useInView(ref, { amount: 0.2, margin: '0px 0px -10% 0px' });
  const shouldAnimate = !prefersReduced && (!pauseWhenOffscreen || inView);

  const coins: Array<{ id: CoinId; t0: number; down: number; side: number }> = [
    { id: 'BTC', t0: 0 * stagger, down: DEFAULT_DOWN.BTC, side: DEFAULT_SIDE.BTC },
    { id: 'DOGE', t0: 1 * stagger, down: DEFAULT_DOWN.DOGE, side: DEFAULT_SIDE.DOGE },
    { id: 'XRP', t0: 2 * stagger, down: DEFAULT_DOWN.XRP, side: DEFAULT_SIDE.XRP },
    { id: 'ADA', t0: 3 * stagger, down: DEFAULT_DOWN.ADA, side: DEFAULT_SIDE.ADA },
    { id: 'ETH', t0: 4 * stagger, down: DEFAULT_DOWN.ETH, side: DEFAULT_SIDE.ETH },
  ];
  ...
  return (
    <LazyMotion features={domMax}>
      <m.svg
        ...
        className={className}
        style={{
          pointerEvents: 'none', // decorative, let card get hover/click
          ...style,
        }}
        initial={{ opacity: 0 }}
      ...
      >
        ...
        {/* Coins (animated or static depending on shouldAnimate) */}
        <g aria-hidden="true">
          {coins.map(({ id, t0, down, side }) => {
            const { x, y, times } = buildKeyframes(t0, down, side, upStart, fallFrac);

            // When we pause, pin at start position (no transform).
            const animate = shouldAnimate
              ? { translateX: x, translateY: y }
              : { translateX: 0, translateY: 0 };
            const transition = shouldAnimate
              ? { duration, ease: 'easeInOut', times, repeat: Infinity }
              : undefined;
