...
import { WalletProvider, useWallet as useDefaultUseWallet } from '@solana/wallet-adapter-react';
...
const DEFAULT_CONTEXT = {
  ...
  signAllTransactions(_transaction: Transaction[]) {
    return Promise.reject(
      console.error(console.error('DEFAULT_WALLET_CONTEXT SIGN_ALL_TRANSACTIONS()'))
    );
  },
  signMessage(_message: Uint8Array) {
    return Promise.reject(console.error('DEFAULT_WALLET_CONTEXT SIGN_MESSAGE()'));
  },
} as WalletContextState;

/*
 * This provider is supposed to handle the custom wallet logic that Normal uses. The primary purpose of this is to get around the weird inconsistencies between wallets and to support the idiosyncrasies in Normal's UIs.
 *
 * The components of the Normal Wallet Provider are:
...
const useHasConnectedWalletBefore = () => {
  const localStorageState = useLocalStorage<boolean>('hasConnectedWalletBefore', false, {
    raw: false,
...
const useCustomAutoconnectLogic = (props: { enabled: boolean; autoconnectionDelay?: number }) => {
  const [walletHasConnectedBefore, setWalletHasConnectedBefore] = useHasConnectedWalletBefore();
  ...
  useEffect(() => {
    ...
    const previouslyConnectedWalletName = window.localStorage[PREFERRED_WALLET_LOCALSTORAGE_KEY];
    ...
    if (walletHasConnectedBefore) {
      ...
      const cleanedPreviousWalletName = previouslyConnectedWalletName.replace(/[^a-zA-Z0-9]/g, '');
      ...
      if (matchingWallet) {
        console.log(`Attempting Autoconnect : ${matchingWallet?.adapter?.name}`);
  ...
  useEffect(() => {
    if (walletContext?.wallet) {
      const connectionHandler = () => {
        console.log(`Handling Connection .. wallet name : ${walletContext.wallet?.adapter.name}`);
      ...
      return () => {
        walletContext.wallet?.adapter.removeListener('connect', connectionHandler);
        walletContext.wallet?.adapter.removeListener('disconnect', disconnectionHandler);
      };
    }
  }, [walletContext?.wallet]);
};

const NormalAutoConnectProvider = (
...
export const NormalWalletContext = React.createContext<WalletContextState>(DEFAULT_CONTEXT);
 ...
 * @returns
 */
const WalletProviderInner = (props: PropsWithChildren<WalletProviderInnerProps>) => {
  ...
  const walletsToShow = useMemo(() => {
    const solanaMobileWalletAdapter = walletContextState.wallets.find(
      (wallet) => wallet.adapter.name === 'Mobile Wallet Adapter'
    );
  ...
  return (
    <NormalWalletContext.Provider value={derivedWalletContext}>
      <NormalAutoConnectProvider
        autoconnectionDelay={props.autoconnectionDelay}
    ...
    </NormalWalletContext.Provider>
  );
};

const NormalWalletProvider = ({
  wallets,
  autoConnect = true,
  disableAutoconnect,
  autoconnectionDelay,
...
export default NormalWalletProvider;
