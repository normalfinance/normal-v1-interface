import type { UIMarket } from '@drift/common';
import type { PublicKey, BulkAccountLoader } from '@drift-labs/sdk';

import { useImmer } from 'use-immer';
import { useInterval } from 'react-use';
import { ENUM_UTILS } from '@drift/common';
import { useRef, useState, useEffect, useCallback } from 'react';
import { BigNum, OracleSource, getOracleClient, PRICE_PRECISION_EXP } from '@drift-labs/sdk';

import { useCommonDriftStore, useOraclePriceStore } from '../../stores';

import type { OraclePriceStore } from '../../stores';
...
const DEFAULT_ORACLE_CLIENTS_MAP: OracleClientsMap = ORACLE_CLIENT_KEYS.reduce((acc, key) => {
...
export const useSyncOraclePriceStore = (
  ...
  const [oracleClients, setOracleClients] = useState<OracleClientsMap>(DEFAULT_ORACLE_CLIENTS_MAP);

  // Keep a local price store state so that the app isn't re-rendering non-stop for every price change
  const [localPriceStoreState, setLocalPriceStoreState] = useImmer<OraclePriceStore['symbolMap']>(
    {}
  );

  const bulkLoaderCallbacks = useRef<[string, PublicKey][]>([]);

  const areOracleClientsReady = Array.from(oracleClients.values()).every((client) => !!client);

  const getMatchingOracleClient = useCallback(
    (oracleSource: OracleSource) => {
      if (!areOracleClientsReady) throw new Error('Oracle clients are not ready');
      ...
      if (!oracleClient) {
        ...
        throw new Error(`Unaccounted for oracle type in useSyncOraclePriceStore.`);
      }
  ...
  useEffect(() => {
    ...
    ORACLE_CLIENT_KEYS.forEach((oracleSourceKey) => {
      ...
      const oracleClient = getOracleClient(oracleSource, connection, driftClient.program);
      setOracleClients((s) => {
  ...
  useEffect(() => {
    ...
    return cleanup;
  }, [connection, areOracleClientsReady, bulkAccountLoader, getMatchingOracleClient]);
    ...
    marketsAndAccounts.forEach(async ({ market, accountToUse }) => {
      const callbackId = await bulkAccountLoader.addAccount(
        ...
        async (accountDataBuffer) => {
          ...
          const accountClient = getMatchingOracleClient(market.market.oracleSource);
          ...
          setLocalPriceStoreState((s) => {
            s[market.key] = {
              market,
              priceData: {
                price: BigNum.from(oraclePriceData.price, PRICE_PRECISION_EXP).toNum(),
                slot: oraclePriceData.slot.toNumber(),
                confidence: BigNum.from(oraclePriceData.confidence, PRICE_PRECISION_EXP).toNum(),
                twap: BigNum.from(oraclePriceData.twap, PRICE_PRECISION_EXP).toNum(),
                twapConfidence: BigNum.from(
                  oraclePriceData.twapConfidence,
                  PRICE_PRECISION_EXP
                ).toNum(),
                maxPrice: oraclePriceData.maxPrice
                  ? BigNum.from(oraclePriceData.maxPrice, PRICE_PRECISION_EXP).toNum()
                  : undefined,
              },
