...
const sumTradingVolumesByBucket = (volumes: TradingVolumeResponse[]): TradingVolumeResponse => {
...
export const API = {
  async getPoolReserveA(contractId: string): Promise<PoolReserveResponse> {
    const { data } = await apiClient.get(`/liquidity/${contractId}/pool_reserve_a`);
    return data;
  },
  async getPoolReserveB(contractId: string): Promise<PoolReserveResponse> {
    const { data } = await apiClient.get(`/liquidity/${contractId}/pool_reserve_b`);
    return data;
  },
  async getPoolShares(contractId: string): Promise<PoolReserveResponse> {
    const { data } = await apiClient.get(`/liquidity/${contractId}/pool_shares`);
    return data;
  },
  ...
  async getTickers(): Promise<TickersResponse> {
    ...
    const uniqueTickers = data.filter(
      (ticker: TickerInfo, index: number, self: TickerInfo[]) =>
        index === self.findIndex((t: TickerInfo) => t.pool_id === ticker.pool_id)
    );
  ...
  async getTradingVolumePerMonth(
    ...
    const params = { from, to };
    const { data } = await apiClient.get(`/trading-vol/${contractId}/permonth`, { params });
    return data;
  },
  ...
  async getAllTradingVolumePerDay(from?: string, to?: string): Promise<TradingVolumeResponse> {
    const tickers = await this.getTickers();
    const volumes = await Promise.all(
      tickers.map((ticker) => this.getTradingVolumePerDay(ticker.pool_id, from, to))
    );
    return sumTradingVolumesByBucket(volumes);
  },

  async getAllTradingVolumePerHour(from?: string, to?: string): Promise<TradingVolumeResponse> {
    const tickers = await this.getTickers();
    const volumes = await Promise.all(
      tickers.map((ticker) => this.getTradingVolumePerHour(ticker.pool_id, from, to))
    );
    return sumTradingVolumesByBucket(volumes);
  },

  async getAllTradingVolumePerWeek(from?: string, to?: string): Promise<TradingVolumeResponse> {
    const tickers = await this.getTickers();
    const volumes = await Promise.all(
      tickers.map((ticker) => this.getTradingVolumePerWeek(ticker.pool_id, from, to))
    );
    return sumTradingVolumesByBucket(volumes);
  },

  async getAllTradingVolumePerMonth(from?: string, to?: string): Promise<TradingVolumeResponse> {
    const tickers = await this.getTickers();
    const volumes = await Promise.all(
      tickers.map((ticker) => this.getTradingVolumePerMonth(ticker.pool_id, from, to))
    );
    return sumTradingVolumesByBucket(volumes);
  },
  async getRatioGraph(contractId: string, from?: string, to?: string): Promise<RatioResponse> {
    const params = { from, to };
