...
function isBreakpointRecord(value: unknown): value is Partial<Record<Breakpoint, string | number>> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export function getSlideSize(slidesToShow: InputValue): InputValue {
  if (isBreakpointRecord(slidesToShow)) {
  ...
  // compiler now sees only string | number
