...
**UPDATE 2026-09-14:** Justin approved email-OTP device enrolment; `POST /api/turnkey/enroll/{init,verify,login,complete}` now exist (see MOBILE_APP_CONTEXT.md §3/§4). The paragraph below describes the state BEFORE that decision.
**Consequence for mobile (pre-2026-09-14):** an existing web user could sign in the app **only** if their passkey was a synced platform credential (iCloud Keychain / Google Password Manager) that the phone can present under rpId `normalfinance.io`. The app cannot register an additional passkey for an existing sub-org, because no add-authenticator flow exists on either side. Users who created their passkey in Chrome on Windows have no path into the app today. Building `CREATE_AUTHENTICATORS_V2` (approved by the existing passkey, from a device that has it) is the fix, and it is new code on web too.
